pipeline {
  ...
  stage ('MVN BUILD') {
    ...
    stage ('STATIC TEST WITH SONAR') {
       steps { 
                sh 'mvn sonar:sonar -Dsonar.projectKey=ProjectDevops -Dsonar.host.url=http://172.10.1.140:9000 -Dsonar.login=7b41f2d88b4f512107a628d74252f22af1f1769d'
      }
    }
  
   stage ('NEXUS DEPLOY') {
       steps {
        sh 'mvn deploy -DaltDeploymentRepository=nexus::default::http://admin:vagrant@172.10.1.140:8081/repository/ProjectDevops/'
